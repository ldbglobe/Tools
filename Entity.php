...
class Entity extends \Model {
	...
	{
		return parent::factory(get_called_class(), $connection_name)->create()->onCreate();
	}
	static function load($id, $connection_name = NULL) // load an ORM instance from BDD and call onLoad hook
	{
		$entity = parent::factory(get_called_class(), $connection_name)->where('id',$id)->find_one();
	...
	{
		return parent::factory($class_name ? $class_name:get_called_class(), $connection_name);
	}
	...
	function onCreate()
	{
		if(static::$_id_mode==='UUID')
		{
			$this->{static::$_id_column} = (string)\Ramsey\Uuid\Uuid::uuid4();
		}
